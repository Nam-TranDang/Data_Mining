...
if __name__ == '__main__':
    ...
    for p,f in [('apriori',  apriori),
        ...
        with open(devnull, 'w') as null:
            for r in range(runs): expats = run_pipe(prg, null)
        stderr.write('done [%.3fs].\n' % (time()-t))

        stderr.write('file   ... '); t = time()
        with open(devnull, 'w') as null:
            for r in range(runs): expats = run_file(prg, null)
        stderr.write('done [%.3fs].\n' % (time()-t))

        pypats = set([(tuple(sorted(list(s))), x) for s,x in pypats])
        expats = set([(tuple(sorted(list(s))), x) for s,x in expats])
