...
{                               /* --- generate a pattern spectrum */
  ...
  int      r;                   /* return value of function call */
  ...
  r = fpg_genpsp(tabag, target, supp, (ITEM)zmin, (ITEM)zmax,
                 FPG_SIMPLE, FPG_DEFAULT, (size_t)cnt, surr, seed,
                 cpus, repfn, &done, &psp);
  if (!r) { pypsp = psp_toPyObj(psp, 1.0/(double)cnt, report[0]);
  ...
  sig_remove();                 /* remove the signal handler */
  if (r == -15) return ERR_VALUE("no (frequent) items found");
  if (r < 0) return ERR_MEM();  /* check for an error */
...
#endif
  r = fpg_estpsp(tabag, target, supp, (ITEM)zmin, (ITEM)zmax,
                 (size_t)equiv, alpha, (size_t)smpls, seed, &psp);
  if (!r) { pypsp = psp_toPyObj(psp, 1/(double)equiv, report[0]);
...
#define FIM_DESC \
  ...
  "version 6.31 (2024.12.13)     (c) 2011-2024   Christian Borgelt"
